...
assert.match(html, /admin\.js\?v=20260906-step-control/);
...
assert.match(app, /\/api\/admin\/audit\?limit=50/);
assert.match(app, /\/api\/admin\/revoke/);
assert.match(app, /Authorization: "tma " \+ initData/);
assert.match(app, /new AbortController\(\)/);
assert.match(app, /tg\.showConfirm/);
assert.match(app, /Legacy owner-доступ активен/);
assert.match(app, /Отозвать этот подарок/);
assert.match(app, /request_id: requestId/);
assert.match(app, /Источник доступа/);
assert.match(app, /Повторный запрос распознан/);
assert.match(app, /Административные действия|Журнал обновлён/);
...
assert.doesNotMatch(app, /var daysLabel\s*=/);
...
assert.match(styles, /\.directory-row/);
assert.match(styles, /\.audit-list/);
